...
export const LeadNotes = ({ leadId, notes = [], onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const note = await leadService.addNote(leadId, trimmed);
      onAdded(note);
      setText('');
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      ...
      {sorted.length === 0 ? (
        <EmptyState icon={StickyNote} title="No notes yet" description="Notes about this lead will show up here." />
      ) : (
