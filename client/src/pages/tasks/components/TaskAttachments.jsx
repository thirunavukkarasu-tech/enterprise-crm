...
export const TaskAttachments = ({ taskId, attachments = [], onUploaded, onRemoved }) => {
  ...
  const handleFileSelected = async (e) => {
    ...
    setProgress(0);
    try {
      const attachment = await taskService.uploadAttachment(taskId, file, setProgress);
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await taskService.removeAttachment(taskId, pendingDelete._id);
  ...
  return (
    <div className="space-y-4">
      ...
      {attachments.length === 0 ? (
        <EmptyState icon={Paperclip} title="No attachments" description="Files attached to this task will show up here." />
      ) : (
      ...
      <ConfirmDialog
        ...
        confirmLabel="Remove"
        description={`Remove "${pendingDelete?.originalName}" from this task?`}
      />
    </div>
  );
};
